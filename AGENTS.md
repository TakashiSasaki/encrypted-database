...
- whether each affected language is baseline-public, baseline-candidate, preview-library, portability-validation, bootstrap-scaffold, or future; (Note: Scaffold-level read/write operations do not qualify as public-quality implementations)
- what tests were run;
...
*   **Python:** Found under `python/`. This is a *baseline implementation*. Uses `cryptography` for crypto ops. Make sure to run `pip install -e .[test]` and run tests using `pytest tests/`. The `created_by_version` metadata is dynamically populated using `importlib.metadata` (falling back to `pyproject.toml`) and is not a compatibility gate. Publication to PyPI is intentionally pending.
*   **Node.js:** Found under `nodejs/`. This is a *baseline implementation*. Uses `better-sqlite3`, native `crypto` module, `argon2`, and `json-canonicalize` (for JCS). Use `pnpm` for package management (e.g., `pnpm install`, `pnpm test`). Never use npm or yarn. The package entrypoint `src/index.js` uses CommonJS to export `EncryptedStorage` and errors, keeping compatibility with `uuid@^9.0.1` and explicitly avoiding ESM conversion. The `created_by_version` metadata is derived from `package.json` and is not a compatibility gate. Publication to npm is intentionally pending.
...
*   **C:** Found under `c/`. This is an **implemented-scaffold** for a bootstrap testing environment. Built with CMake, it must not use package managers like Conan or vcpkg. Tested by running `cmake -S . -B build`, `cmake --build build`, and `ctest --test-dir build --output-on-failure`. It is *not* a production storage library.
*   **C++:** Found under `cpp/`. This is an **implemented-scaffold** for a bootstrap testing environment. Built with CMake, it must not use package managers like Conan or vcpkg. Tested by running `cmake -S . -B build`, `cmake --build build`, and `ctest --test-dir build --output-on-failure`. C and C++ are independent native scaffold implementations. They are *not* production storage libraries.

### C/C++ Generic JCS Architecture
The C/C++ Generic JCS implementation defines a 7-layer boundary architecture (Layers A-G) spanning generated-AST bridge tests, parser-free in-memory models, serializers, test-only generic positive loaders, build-time generated positive fixture wiring, and explicitly deferred future raw parsers and public APIs.
*   **Parser-free JCS internal model:** Implemented in `cpp/src/vault_jcs_model.cpp` (via `ModelValue::serialize()`) and C equivalents. They pre-validate all object keys and string values for valid UTF-8, failing closed on invalid sequences. Keys are sorted via a hardened UTF-16 code unit comparator without implicit byte-ordering fallbacks. The planned UTF-16 comparator strategy uses UTF-8 to UTF-16 code units, fails closed on invalid Unicode, and strictly avoids normalization, `wchar_t`, or `codecvt`.
*   **Generic positive vector loaders:** These are test-harness utilities using length-aware test-only generic JSON nodes (e.g., `VaultJcsTestBytes` in C). They do not parse JSON text and enforce strict fail-closed validation for unsupported inputs such as floats, duplicates, embedded NULs, and out-of-bounds integers (beyond `[-9007199254740991, 9007199254740991]`).
*   **Build-time fixture generation:** Python scripts filter approved JSON vector files (`rfc8785-basic.json`, `utf16-key-ordering.json`, `generic-positive-coverage.json`) and generate safe test fixtures directly into the build directory (not checked into the repo). `scripts/generate_jcs_positive_loader_fixtures.py` enforces a strict dictionary schema requiring exactly 'name', 'description', 'input', 'expected_string', and 'expected_hex'.
