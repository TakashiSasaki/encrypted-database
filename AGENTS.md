...
*   **Cross-language Validation:** Cross-language read/write compatibility testing is planned to use non-public, test-only wrappers (e.g., compatibility_wrapper.py, compatibility_wrapper.js) that accept parameters via CLI arguments/stdin/env and output machine-readable JSON results on stdout to the compatibility runner. The baseline compatibility pairs currently skip tests because these wrappers and the test fixture contracts are not yet implemented.
