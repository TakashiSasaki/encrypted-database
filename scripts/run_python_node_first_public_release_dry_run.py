...
def main():
    ...
    pub_ready = pub_data.get("publication_ready", False) if pub_data else False
    publication_authorized = pub_data.get("publication_authorized", False) if pub_data else False

    # Aggregated readiness needs all requested gates to pass
    publication_ready = (
        pub_ready and
        pf_passed and
        rc_passed and
        (not args.include_installed_matrix or im_passed)
    )
