...
class TestRunCrossLanguageCompatibility(unittest.TestCase):
    ...
    @unittest.skipUnless(os.environ.get("VAULT_RUN_COMPAT_EXECUTION_TESTS") == "1", "Gated behind VAULT_RUN_COMPAT_EXECUTION_TESTS=1")
    def test_execution_mode_direct_api(self):
        result = subprocess.run(
            ["python", "scripts/run_cross_language_compatibility.py", "--execute", "--mode", "direct-public-api"],
            capture_output=True, text=True
        )
        self.assertEqual(result.returncode, 0)
        out = result.stdout
    ...
    @unittest.skipUnless(os.environ.get("VAULT_RUN_COMPAT_EXECUTION_TESTS") == "1", "Gated behind VAULT_RUN_COMPAT_EXECUTION_TESTS=1")
    def test_execution_mode_direct_api_json(self):
        result = subprocess.run(
            ["python", "scripts/run_cross_language_compatibility.py", "--execute", "--mode", "direct-public-api", "--json"],
            capture_output=True, text=True
        )
        self.assertEqual(result.returncode, 0)
        data = json.loads(result.stdout)
