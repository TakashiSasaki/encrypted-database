...
def execute_pair(writer, reader):
    ...
    evidence = {
        ...
        "wrapper_writer": LANGUAGES[writer]["wrapper"],
        "wrapper_reader": LANGUAGES[reader]["wrapper"],
        "database": "temporary-file",
        "artifact_policy": "not committed"
    }

    return "passed", "Successfully executed test-wrapper compatibility checks", evidence
...
def main():
    ...
    for w, r in pairs_to_run:
        ...
        results.append({
            ...
            "status": status,
            "reason": reason,
            "evidence": evidence if args.execute else None
        })
