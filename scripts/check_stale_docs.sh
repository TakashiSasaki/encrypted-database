...
check_phrase() {
    local pattern="$1"
    local desc="$2"
    # Find matching lines but ignore this script itself
    local matches=$(grep -rnEi "$pattern" docs/ go/ rust/ python/ nodejs/ browser-test/ --exclude="check_stale_docs.sh" 2>/dev/null || true)
    if [ -n "$matches" ]; then
