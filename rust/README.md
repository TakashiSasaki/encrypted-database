...
The Rust implementation is not a full, production-ready library. Currently, a minimal JCS canonicalizer, an Argon2id KDF test, AES-256-GCM AEAD primitive vector validation, and Key-Wrap conformance validation are implemented solely to satisfy the basic AAD, KDF, AEAD, and Key-Wrap test vectors. Full storage database features, including SQLite validation, unlock, decrypt, and writer functionality, are future tasks.
