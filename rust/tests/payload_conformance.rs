...
fn test_payload_conformance() {
    ...
    let content = fs::read_to_string(path).expect("Failed to read payload vector file");
    let vecs: Vec<Value> = serde_json::from_str(&content).expect("Failed to parse vector JSON");

    for vec in vecs {
        let name = vec["name"].as_str().unwrap_or("unknown");
        let valid = vec["valid"].as_bool().unwrap_or(false);

        let alg = match vec.get("alg").and_then(|v| v.as_str()) {
            Some(a) => a,
            None => {
                if valid {
        ...
        if alg != "A256GCM" {
            if valid {
                ...
                println!(
                    "Skipping unsupported alg {} in negative test: {}",
                    alg, name
                );
                continue;
            }
        ...
        let key_hex = match vec.get("record_dek_hex").and_then(|v| v.as_str()) {
            Some(h) => h,
            None => {
                if valid {
        ...
        let nonce_hex = match vec.get("nonce_hex").and_then(|v| v.as_str()) {
            Some(h) => h,
            None => {
                if valid {
        ...
        if let Some(tag_hex) = vec.get("expected_tag_hex").and_then(|v| v.as_str()) {
        ...
        let object_uuid = vec.get("object_uuid").and_then(|v| v.as_str());
        let schema_uuid = vec.get("schema_uuid").and_then(|v| v.as_str());
        let content_type = vec.get("content_type").and_then(|v| v.as_str());
        let kid = vec.get("kid").and_then(|v| v.as_str());

        if object_uuid.is_none() || schema_uuid.is_none() || content_type.is_none() || kid.is_none()
        {
            if valid {
        ...
        if let Some(expected_aad_hex) = vec.get("expected_aad_hex").and_then(|v| v.as_str()) {
        ...
        let payload_json = match vec.get("payload_json") {
            Some(p) => p,
            None => {
                if valid {
        ...
        if let Some(expected_payload_jcs_hex) =
            vec.get("expected_payload_jcs_hex").and_then(|v| v.as_str())
        {
        ...
        let has_expected_ct = if let Some(ct_hex) = vec
            .get("expected_ciphertext_and_tag_hex")
            .and_then(|v| v.as_str())
        {
        ...
        if valid {
            ...
            // RustCrypto Aes256Gcm returns ciphertext || tag
            if has_expected_ct {
                assert_eq!(
