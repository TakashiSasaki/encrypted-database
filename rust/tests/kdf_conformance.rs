...
fn test_argon2id_kdf_vectors() {
    ...
    let data = std::fs::read_to_string(vector_path).expect("Failed to read KDF vector file");
    let cases: Vec<Argon2idTestCase> =
        serde_json::from_str(&data).expect("Failed to parse KDF vector file as JSON");

    for (i, tc) in cases.iter().enumerate() {
        assert_eq!(
            tc.profile, "argon2id-profile-v1",
            "Unsupported profile in test case {}",
            i
        );
        ...
        assert!(
            !(has_hex && has_b64),
            "Test case {}: ambiguous input, both salt_hex and salt present",
            i
        );
        ...
        assert_eq!(
            salt_bytes.len(),
            tc.parameters.salt_bytes,
            "Decoded salt length does not match expected length in test case {}",
            i
        );
        ...
        assert_eq!(
            derived_key.len(),
            output_len,
            "Derived key length does not match expected length in test case {}",
            i
        );

        let derived_hex = hex::encode(derived_key);
        assert_eq!(
            derived_hex, tc.expected_output_hex,
            "Derived key does not match expected output in test case {}",
            i
        );
    }
}
