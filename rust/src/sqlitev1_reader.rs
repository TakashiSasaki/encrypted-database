...
pub enum ReaderError {
    ...
    ObjectNotFound,
    #[error("Record DEK wrap info not found")]
    WrapInfoNotFound,
...
impl ReadOnlyReader {
    fn unlock_database(&mut self, passphrase: &str) -> Result<(), ReaderError> {
        ...
        let db_kid: String = stmt
            ...
            .optional()?
            .ok_or(ReaderError::Locked)?;

        let mut stmt = self.conn.prepare(
            "SELECT wrapping_kid, nonce, wrapped_key, aad_policy FROM wrapped_key_tbl WHERE wrapped_kid = ?",
        )?;
            ...
            if let Some((provider, config_json)) = unlock_row {
                if provider == "passphrase_argon2id" {
                    let config: Value = serde_json::from_str(&config_json)
                        .map_err(|_| ReaderError::InvalidFormat("Invalid JSON".to_string()))?;

                    let canonical_config = canonicalize(&config)
                        .map_err(|e| ReaderError::InvalidFormat(format!("JCS failed: {}", e)))?;
                    ...
                    let mem_kib = config["memory_kib"].as_u64().ok_or_else(|| {
                        ReaderError::InvalidFormat("Missing memory_kib".to_string())
                    })?;
                    let iter = config["iterations"].as_u64().ok_or_else(|| {
                        ReaderError::InvalidFormat("Missing iterations".to_string())
                    })?;
                    let parallelism = config["parallelism"].as_u64().ok_or_else(|| {
                        ReaderError::InvalidFormat("Missing parallelism".to_string())
                    })?;
                    let out_bytes = config["output_bytes"].as_u64().ok_or_else(|| {
                        ReaderError::InvalidFormat("Missing output_bytes".to_string())
                    })?;
                    let salt_b64 = config["salt"]
                        .as_str()
                        .ok_or_else(|| ReaderError::InvalidFormat("Missing salt".to_string()))?;
                    ...
                    let salt = decode_strict(salt_b64)
                        .map_err(|e| ReaderError::InvalidFormat(format!("Salt decode: {}", e)))?;
                    ...
                    let mut unlock_kek = vec![0u8; out_bytes as usize];
                    let params = Params::new(
                        mem_kib as u32,
                        iter as u32,
                        parallelism as u32,
                        Some(out_bytes as usize),
                    )
