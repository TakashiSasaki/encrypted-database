...
| FINDING-007 | 2026-05-25 | SQLite | Go/Rust | `database_uuid` validation requires strict enforcement of RFC4122 version and variant bits rather than just allowing any canonical 8-4-4-4-12 hex shape. This prevents cross-language portability divergence by ensuring invalid variants are rejected explicitly across all implementations. | clarification | Low | Restored the stricter pattern in Rust and Go to validate UUID version and variant bits. | Active | `go/internal/sqlitev1/validator.go`, `rust/src/sqlitev1.rs` |
