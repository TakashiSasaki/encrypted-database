...
- **It is an internal test-harness contract.** The generated AST data structure is only for feeding test vectors into the native implementation serializers. It acts as a seed for future generic JCS conformance planning and may inform a future internal value model, but is not automatically that final model. It is not a replacement for a generic parser or canonicalizer design (see the [C/C++ JCS Parser and Dependency Decision](./c-cpp-jcs-parser-dependency-decision.md)). The future generic model boundary is defined in the [C/C++ JCS Internal Value Model Decision](./c-cpp-jcs-internal-value-model-decision.md).
