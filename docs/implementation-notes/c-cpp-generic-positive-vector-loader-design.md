...
- The Python generator enforces strict schema validation (exactly five allowed keys, strict type matching for metadata fields). Generator contract test coverage has been added.
...
- All current vectors in `test-vectors/jcs/rfc8785-basic.json` are wired through the build-time generated generic positive fixture path. `rfc8785-basic.json` remains the active generated-AST suite as well.
- The new `test-vectors/jcs/generic-positive-coverage.json` vector source is added. It is a safe positive-only vector file and is consumed by the build-time generic positive fixture generator.
