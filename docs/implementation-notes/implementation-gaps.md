...
**Status:** Active
**Area:** Distribution
**Current state:** Python/Node.js are baseline-public certified. A non-publishing Phase 8 distribution preflight script (`scripts/run_python_node_release_preflight.py`) is implemented and runs successfully to verify wheel/sdist and npm pack artifact generation, clean install, and smoke tests. Phase 9 installed-distribution matrix and artifact hash manifest generation are complete and passing. Phase 11 (Release Candidate Freeze), Phase 12 (Execution Readiness), Phase 13 (Human Decision Gate), and Phase 14 (Dry-Run Registry Readiness) are complete. Actual PyPI/npm publication remains pending. Release automation requiring secrets remains pending. Namespace/account decisions must be resolved.
...
**Recommended next action:** Decide on package namespace (e.g. `encrypted-storage`), define secure credential workflows, obtain explicit sign-off on the decision record, and perform controlled public package release.
...
**Status:** Active
**Area:** Implementation / Portability
**Current state:** C and C++ are independent bootstrap/native conformance scaffolds with smoke tests, partial AAD shared-vector conformance, internal JSON escaping for AAD construction, a limited internal generated-AST JCS basic-vector serializer scaffold, an initial C parser-free JCS internal model scaffold (including arrays, objects, serializer hardening, and a hardened generated-vector bridge) and an initial C++ parser-free JCS internal model scaffold (including serializer hardening and a generated-vector bridge), internal UUID syntax validation, and internal content-type boundary validation. A parser/dependency strategy and an internal value model boundary are now accepted for future generic JCS work. They do not implement full generic JCS, an arbitrary JSON parser, Storage Format V1 cryptography, SQLite reader/writer support, production public APIs, or matrix integration. The architecture has been explicitly decided: C and C++ are independent implementations.
...
**Recommended next action:** A generic JCS implementation strategy is now accepted. Future work should resolve number/sorting boundaries by adding boundary vectors, implement generic JCS per the accepted decision, and then continue with cryptographic primitive vector conformance.
