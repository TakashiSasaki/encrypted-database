...
**Status:** Active
**Area:** Testing / CI
**Current state:** GitHub Actions workflows generate and upload coverage artifacts (`coverage.xml`, `lcov.info`), but we do not yet have a reliable, non-polluting mechanism configured to host a coverage summary JSON endpoint for `shields.io` badges using GitHub Pages without interfering with the active branch deployments.
**Expected or intended state:** Coverage badges correctly display on the `README.md` via a static endpoint generated by the CI.
**Why it matters:** Good visibility into CI test coverage encourages maintainability and testing standards.
**Recommended next action:** Establish a dedicated gh-pages artifact deployment path or a Gist-based updater to safely host the coverage summary endpoint.
