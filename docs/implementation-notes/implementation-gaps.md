...
**Status:** Active
**Area:** Implementation / Portability
**Current state:** C and C++ are independent bootstrap/native conformance scaffolds with smoke tests, partial AAD shared-vector conformance, internal JSON escaping for AAD construction, a limited internal generated-AST JCS basic-vector serializer scaffold, internal UUID syntax validation, and internal content-type boundary validation. A parser/dependency strategy is now proposed for future generic JCS work. They do not implement full generic JCS, an arbitrary JSON parser, Storage Format V1 cryptography, SQLite reader/writer support, production public APIs, or matrix integration. The architecture has been explicitly decided: C and C++ are independent implementations.
