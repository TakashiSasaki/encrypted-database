...
**Next-Phase Decision Surface:** Public API parity remains incomplete across languages. The main visible asymmetry is that Go/Rust writer scaffolds expose update/delete functionality, while Python/Node.js baseline APIs do not. The next design decision should determine whether to add public update/delete APIs to Python/Node.js or explicitly defer it as intentional non-parity. (Update/delete APIs are not implemented in Python/Node.js as part of the V1 Stable milestone).
...
**Status:** Active
**Area:** Cryptography / Interoperability
**Current state:** Go/Rust writer scaffolds exist. They implement database creation and payload insertion. A write-matrix harness validates 8 combinations (Go/Rust writer scaffolds vs Go/Rust/Python/Node readers) and passes locally. A GitHub Actions workflow for this write-matrix has been added and runs on path-filtered `pull_request` and `push` events (as well as `workflow_dispatch`). However, Go/Rust writer scaffolds are strictly for validation and are not full production storage libraries. Update/delete APIs exist in the current scaffold and are covered by write-matrix update/delete scenarios. Python, Node.js, and browser-test baseline libraries have also implemented update/delete APIs for public API parity. Key lifecycle work (key rotation, decrypt-only migration, key destruction, rewrap), additional unlock providers, blind index, and full production public API maturity remain future work. (Note: The incompleteness of Go/Rust APIs does not weaken the Storage Format V1 Stable status).
