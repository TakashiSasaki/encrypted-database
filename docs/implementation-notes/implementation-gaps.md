...
**Status:** Active
**Area:** Distribution
**Current state:** Python/Node.js are baseline-public certified. A non-publishing Phase 8 distribution preflight script (`scripts/run_python_node_release_preflight.py`) is implemented and runs successfully to verify wheel/sdist and npm pack artifact generation, clean install, and smoke tests. Phase 9 installed-distribution matrix and artifact hash manifest generation are complete and passing. Phase 10 governance and non-secret publication readiness is currently active. Actual PyPI/npm publication remains pending. Release automation requiring secrets remains pending. Namespace/account decisions must be resolved.
