...
1. `kid` を UUIDv4 とする場合の生成責務。ライブラリ内部で常に生成するか、呼び出し側が生成済み UUIDv4 を渡せるようにするかを決める。

2. UUIDv4 の表記形式。小文字ハイフン付き canonical form、すなわち `xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx` に固定するか、SQLite 内部では 16 byte BLOB とし、外部表現だけ文字列にするかを決める。

3. JSON 正規化方式の厳密仕様。RFC 8785 JCS 準拠とするか、Python 実装の `sort_keys=True`, `separators=(",", ":")`, UTF-8 固定程度の独自 canonical JSON とするかを決める。相互運用を考えるなら RFC 8785 準拠が望ましい。

4. 正規化 JSON の保存方針。DB に保存する `description_json`, `provider_config_json`, `aad_context_json` を、挿入時に必ず正規化済み文字列へ変換して保存するか、読み込み時に正規化して比較するかを決める。推奨は保存時正規化である。

5. `provider_config_json` の schema registry を DB 内に持つか、コード側に固定するか。DB 内に `config_schema_id` と schema 本体を保存すると自己記述性は上がるが、検証実装は複雑になる。

6. `wrapped_key_tbl` の主キー設計。現在の `PRIMARY KEY (wrapped_kid, wrapping_kid)` では、同じ鍵ペアの複数世代 wrap を保持しにくい。履歴保持、ローテーション、再 wrap 失敗時の安全性を考えるなら、`wrap_id` UUIDv4 を導入する案を検討する。

7. `wrapped_key_tbl` の状態管理。wrap 済み鍵にも `status`, `created_at_ms`, `deactivated_at_ms`, `destroyed_at_ms` を持たせるかを決める。`key_tbl.status` だけでは、同じ鍵の複数 unwrap 経路の有効・無効を個別管理しにくい。

8. `encrypted_object_tbl` の更新履歴。上書き更新のみとするか、暗号文バージョンを履歴として保持するかを決める。同期や監査を重視するなら object revision table を別に設ける余地がある。

9. `workspace_kek` を初期実装に含めるか。共有、プロジェクト境界、コレクション境界が当面不要なら後回しにできる。

10. `device_tbl` を導入するか。OS secret store や hardware token provider は端末依存になるため、`unlock_kek_tbl.device_id` の参照先として device table を持つ方がよい可能性がある。

11. `platform_tbl` の粒度。`windows`, `macos`, `linux`, `android`, `ios`, `web`, `server`, `cloud` で十分か、`windows_service`, `linux_headless`, `wsl`, `browser_extension` などを別 platform として扱うかを決める。

12. `unlock_provider_policy_tbl` の導入。provider の定義と、アプリ・組織・platform ごとの許可/禁止/優先順位を分けるかを決める。

13. `test_only` provider の本番無効化方法。DB 制約で防ぐか、ビルド設定・実行時 policy・アプリケーション検証で防ぐかを決める。

14. `index_key` と blind index の詳細。どのフィールドに blind index を許すか、低エントロピー値を禁止するか、index key のローテーションをどう扱うかを決める。

15. AAD policy の versioning。`record-payload-v1`, `wrap-record-key-v1`, `wrap-database-key-v1` などの AAD policy をどこで定義し、どう移行するかを決める。

16. `alg` と `key_profile_tbl` の整合性検証。envelope 側の `alg`、`key_tbl.alg`、`key_profile_tbl.alg` が不一致のときに必ず拒否する実装規則を明文化する。

17. AEAD アルゴリズムの拡張方針。初期標準を `A256GCM` としつつ、将来的に `XCHACHA20-POLY1305` や platform native key handle の unwrap 方式をどう表現するかを決める。

18. nonce 生成・衝突対策。ランダム 96-bit nonce を標準とする場合の暗号化回数上限、鍵ローテーション基準、テスト用 deterministic nonce の禁止を明文化する。

19. エラー分類。認証タグ不一致、鍵未発見、provider 利用不可、policy 違反、JSON schema 不正、AAD 不一致を内部的に区別しつつ、外部には過剰な情報を出さない方針を定義する。

20. ログ方針。`kid`, `object_uuid`, `schema_uuid`, provider 名はログ出力可能か、nonce や ciphertext の扱い、平文 payload と鍵 bytes の絶対禁止を明文化する。

21. メモリ上の秘密管理。Python 実装では秘密鍵 bytes の完全なゼロ化が難しいため、脅威モデル上の限界と、可能な範囲の短寿命化・スコープ限定・ログ禁止を明記する。

22. バックアップと復元。SQLite ファイル、OS secret store、Shamir share、復旧コード、KMS policy のどれが失われると復旧不能になるかを明文化し、復旧テスト手順を用意する。

23. 鍵破棄の意味論。`status = destroyed` は台帳上の論理状態であり、実際の鍵材料や wrap 済み鍵 blob を消すのか、消した場合に参照整合性をどう保つかを決める。

24. 暗号化 payload の粒度。JSON object 全体を暗号化するか、フィールド単位の `EncryptedValue` も許すかを決める。検索・差分同期・漏洩メタデータのバランスに影響する。

25. 添付ファイル・大容量 BLOB の扱い。`file_dek`, chunk 暗号化、ストリーミング AEAD、chunk nonce、ファイルメタデータ暗号化の仕様を別途定義する必要がある。

26. 監査ログ。鍵作成、wrap 作成、unlock provider 追加、provider 無効化、鍵破棄、復旧実行などを event log に記録するかを決める。

27. 互換性と migration。schema version、envelope version、provider config schema version、AAD policy version をどの単位で管理し、どの順序で migration するかを決める。

### 12.3 追加で検討すべき論点

以下はこれまで明示していなかったが、設計上重要になる可能性がある。

1. threat model の明文化。DB ファイル単体漏洩、同一 OS ユーザー権限のマルウェア、管理者権限攻撃、メモリダンプ、バックアップ漏洩、クラウド同期漏洩をどこまで守るかを明確にする。

2. metadata leakage の許容範囲。`object_uuid`, `schema_uuid`, `content_type`, `created_at_ms`, `updated_at_ms`, `key_class`, `purpose`, provider 名などは平文で残る。これらが漏洩してよいかを確認する。

3. 同期・複製時の鍵共有モデル。複数端末で同じ DB を使う場合、端末ごとの `unlock_kek` をどう追加し、失効し、同期競合を解決するかを決める。

4. パスワード変更と provider 追加の原子性。`database_kek` の wrap 行を追加・削除する処理が途中で失敗した場合に、復旧不能にならない transaction 設計が必要である。

5. KDF パラメータ更新。Argon2id の memory / iterations を将来引き上げるとき、既存 `database_kek` を再 wrap するだけで済むように migration 手順を設計する。

6. OS secret store の障害時フォールバック。DPAPI / Keychain / Android Keystore の secret が失われた場合に、passphrase 経路または recovery 経路で復旧できることを UI と運用で保証する。

7. hardware token の user presence と rate limit。PIV, FIDO2, PKCS#11 などでは PIN、touch、retry counter、lockout が存在するため、provider_config とエラー処理に反映する必要がある。

8. KMS 使用時の online dependency。リモート KMS provider ではオフライン利用不可、権限失効、監査ログ、リージョン障害、API rate limit を考慮する。

9. 暗号文の可搬性。DB を別アプリ・別言語で読めるようにするか、Python 実装専用でよいかにより、JSON 正規化、base64url、BLOB、日時形式の厳密さが変わる。

10. 失敗時の UX。パスワード誤り、OS unlock 失敗、hardware token 未接続、Shamir share 不足をどう区別してユーザーに見せるかを決める。

11. 検証用 test vector。鍵、nonce、AAD、plaintext、ciphertext の固定 test vector を用意し、実装間互換性を担保する。

12. セキュリティレビュー境界。暗号プリミティブ自体は既存ライブラリに委譲し、このライブラリでは envelope、鍵階層、AAD、DB 制約、provider 抽象化をレビュー対象にする。
