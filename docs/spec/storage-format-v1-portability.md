...
Zig currently has a generalized read-only reader scaffold and writer scaffold with shared-vector conformance.
...
- SQLite V1 metadata validation
- SQLite V1 generalized read-only reader scaffold
...
Additionally, minimal C and C++ bootstrap scaffolds have been introduced to establish directory structures and basic CI/build conventions. They are independent native scaffold implementations that currently provide partial AAD shared-vector conformance, internal JSON escaping, a limited generated-AST JCS basic-vector serializer scaffold, and internal UUID/content-type boundary validation. They do not implement full Storage Format V1 cryptography, SQLite reader/writer support, matrix integration, or production public APIs. They share the same Storage Format V1 spec, shared vectors, and conformance expectations. C++ is not planned as a wrapper around C. This does not make either implementation production-ready.
