...
None. The schema now includes `storage_metadata_tbl`, PRAGMA checks, and the necessary `CHECK` constraints for `nonce`, `wrapped_key`, `ciphertext`, and `content_type`. (Note: Browser-test environments using `sql.js` bypass PRAGMA application_id / user_version validations and malformed-on-disk corruption checks via `ignore_check_constraints` as documented testing exceptions, but fully enforce storage-format metadata, feature flags, provider_config semantics, and JCS canonicality at the application layer).
