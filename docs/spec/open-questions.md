...
*   **UUIDv4 Validation Location:** [Partially Resolved] ADR-0001 defines the format. The canonical lowercase hyphen-separated string format is currently enforced via library validation and strict SQLite `CHECK` constraints in the schema. Whether 16-byte BLOBs could be used internally within SQLite is still deferred to future V2 considerations.
*   **JSON Canonicalization Compliance:** [Partially Resolved] RFC 8785 JCS is accepted as the standard (ADR-0002) and is implemented for validated primitive JCS shapes within V1 core paths (e.g. `provider_config_json`). Full strict RFC 8785 compliance in Python and Node.js baseline implementations remains an ongoing effort, and resolving whether any legacy non-strict canonical JSON approaches (like Python's `sort_keys=True`, `separators=(",", ":")`) need backwards compatibility during migration remains an open question.
...
*   **Verification Test Vectors:** [Partially Resolved] Extensive JSON test vectors for JCS, AAD, KDF, AEAD, key-wrap, and payload encryption have been generated and implemented across Python, Node.js, and browser-test baselines, as well as Go/Rust portability validation scaffolds. Expanding these vectors to full broader API coverage remains an open task.
