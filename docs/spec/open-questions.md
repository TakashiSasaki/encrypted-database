...
## 1. Resolved but requiring further specification

*   **`kid` Generation:** With `kid` defined as UUIDv4, it needs to be decided whether the library internally generates the UUIDv4 or if the caller is allowed to provide a pre-generated UUIDv4.
*   **UUIDv4 Formatting:** Whether to enforce the canonical lowercase hyphen-separated format strictly as strings everywhere, or to use 16-byte BLOBs internally within SQLite and only use strings for external representation.
*   **JSON Canonicalization Strictness:** Whether to strictly adhere to RFC 8785 JCS, or maintain a custom canonical JSON approach (like Python's `sort_keys=True`, `separators=(",", ":")`, UTF-8 fixed). Strict RFC 8785 is preferred for interoperability.
*   **Normalization Timing:** Whether `description_json`, `provider_config_json`, and `aad_context_json` should be normalized *before* saving to the DB, or if they are normalized upon reading. (Save-time normalization is recommended).
