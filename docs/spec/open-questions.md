...
*   **JSON Canonicalization Compliance:** [Partially Resolved] RFC 8785 JCS is accepted as the standard (ADR-0002) and is fully implemented in V1 core paths (e.g. `provider_config_json`, payload canonicalization). Resolving whether any legacy non-strict canonical JSON approaches (like Python's `sort_keys=True`, `separators=(",", ":")`) need backwards compatibility during migration remains an open question for full production-grade multi-language implementations.
