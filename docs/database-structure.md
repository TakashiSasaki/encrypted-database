...
This section documents **writer defaults and required connection settings** for SQLite backends. It does not replace the normative requirements in [`docs/spec/storage-format-sqlite.md`](spec/storage-format-sqlite.md).

### Required per-connection setting (normative)

* `PRAGMA foreign_keys=ON`
  * Required by the SQLite profile for V1 conformance.
  * Must be executed immediately on each connection used for reads/writes.
