...
This directory contains the Go portability validation and writer scaffold for Storage Format V1 Stable. It is intended to consume shared conformance vectors and uncover portability issues.
...
* In-place payload update (`UpdatePayload`) and payload delete (`DeletePayload`) for `encrypted_object_tbl` rows only.
